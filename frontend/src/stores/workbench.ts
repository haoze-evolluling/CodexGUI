...
export const useWorkbenchStore = defineStore('workbench', {
  ...
  actions: {
    ...
    async useProvider(action: Promise<any>) { const result = await action; if (!result.ok) throw new Error(result.error || '操作失败。'); this.providers = { ...result.state, providers: result.state?.providers ?? [] } },
  },
})
