...
export const ActivityItem = memo(function ActivityItem({
  ...
  if (activity.type === 'plan_decision') {
    if (activity.status === 'answered') {
      return (
        <section className="plan-decision plan-decision-answered" aria-label="已执行的计划选项">
          <span className="answered"><Check size={15} /> 已选择：{activity.choice ? planChoiceLabel[activity.choice] : '继续执行计划'}</span>
        </section>
      );
    }
    return (
    ...
    return (
      <section className="user-input-activity">
        {activity.questions.map(question => (
          <fieldset key={question.id} disabled={answered || cancelled}>
        ...
        {answered ? <span className="answered"><Check size={15} /> 已提交</span> : cancelled ? <span className="answered">已取消</span> : (
          ...
          <section className="file-change" key={file.path}>
            ...
            {file.diff ? (
              <>
                ...
                {file.diffTruncated && <button type="button" onClick={() => onLoadDiff?.(activity.id, file)}>加载完整差异</button>}
              </>
            ) : file.diffTruncated ? <button type="button" onClick={() => onLoadDiff?.(activity.id, file)}>加载差异</button> : <p>没有可显示的差异。</p>}
          </section>
        ))}
      </div>
    </details>
