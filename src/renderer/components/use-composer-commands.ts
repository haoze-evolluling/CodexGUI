...
import { AtSign, Bot, BrainCircuit, CircleGauge, FilePlus2, ListTodo, Minimize2, Send, ShieldCheck, Sparkles, Undo2 } from 'lucide-react';
...
type Options = Pick<ComposerProps, 'activeSessionId' | 'collaborationModes' | 'input' | 'onChooseFiles' | 'onCompact' | 'onInputChange' | 'onModeChange' | 'onNewConversation' | 'onRollback' | 'onSend' | 'onShowStatus' | 'onSkillSelect' | 'selectedSkill' | 'session' | 'skills'> & {
  disabled: boolean;
...
export function useComposerCommands(options: Options) {
  ...
  const commands = useMemo(() => [
    { kind: 'command' as const, id: 'continue', name: '继续', shortcut: '/continue', description: '向智能体发送续作提示词', icon: Send, disabled: options.disabled, run: () => options.onSend(continuePrompt) },
    ...
    { kind: 'command' as const, id: 'permissions', name: '权限设置', shortcut: '/permissions', description: '设置后续请求的默认权限模式', icon: ShieldCheck, disabled: options.disabled, run: () => options.setOpenSelector('permission') },
  ...
  useEffect(() => {
    let current = true;
    const listMentionFiles = options.listMentionFiles;
    if (!mentionActive || !options.session?.cwd || !listMentionFiles) {
      setMentionFiles([]);
    ...
    const timer = window.setTimeout(() => {
      listMentionFiles(options.session!.cwd, mentionQuery).then(files => {
        if (!current) return;
