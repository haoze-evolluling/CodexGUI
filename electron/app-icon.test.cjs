...
test('main process wires AUMID, window icon, and notification icon', () => {
  const main = fs.readFileSync(path.join(__dirname, 'main.cjs'), 'utf8');
  assert.match(main, /const APP_ID = 'com\.leeha\.codexgui';/);
  assert.match(main, /app\.setAppUserModelId\(APP_ID\);/);
  assert.match(main, /const APP_ICON = path\.join\(__dirname, 'assets', 'app-icon\.ico'\);/);
  const occurrences = main.split('APP_ICON').length - 1;
  assert.ok(occurrences >= 3, `APP_ICON used at least 3 times (got ${occurrences})`);
});
